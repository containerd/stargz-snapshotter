...
REGISTRY_NETWORK=kind_cri_registry_network
DUMMYUSER=dummyuser
DUMMYPASS=dummypass
TESTIMAGE_ORIGIN="ghcr.io/stargz-containers/ubuntu:20.04"
TESTIMAGE="${REGISTRY_HOST}:5000/library/ubuntu:20.04"
KIND_CLUSTER_NAME=kind-cri-stargz-snapshotter
PREPARE_NODE_NAME="kind-cri-prepare-node"
PREPARE_NODE_IMAGE="kind-cri-prepare-image"
...
services:
  ...
  image-prepare:
    ...
    volumes:
    - "criauth-prepare-containerd-data:/var/lib/containerd"
    - "criauth-prepare-containerd-stargz-grpc-data:/var/lib/containerd-stargz-grpc"
...
volumes:
  ...
  criauth-prepare-containerd-stargz-grpc-data:
networks:
  default:
    external:
