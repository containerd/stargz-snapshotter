...
COPY ./config/config.stargz.toml /etc/registry-storage/config.toml
...
services:
  ${BENCHMARKING_NODE}:
    ...
    - "additional-store-data:/var/lib/registry-storage:delegated"
volumes:
  containers-data:
...
LOG_FILE="${LOG_DIR}/registry-storage-benchmark-$(date '+%Y%m%d%H%M%S')"
touch "${LOG_FILE}"
...
if ! ( cd "${CONTEXT}" && \
           ...
           docker exec -e BENCHMARK_SAMPLES_NUM -i "${BENCHMARKING_CONTAINER}" \
                  script/benchmark2/hello-bench/run.sh \
