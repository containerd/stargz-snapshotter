...
func (sf *file) processBatchChunks(args *batchWorkerArgs) {
	defer args.wg.Done()
	...
	for chunkIdx := args.workerID; chunkIdx < len(args.chunks); chunkIdx += args.workerCount {
		...
		if _, err := sf.fr.ReadAt(bufStart, chunk.offset); err != nil && err != io.EOF {
			args.errChan <- fmt.Errorf("failed to read data at offset %d: %w", chunk.offset, err)
			return
		}

		if err := sf.gr.verifyOneChunk(sf.id, bufStart, chunk.digestStr); err != nil {
			args.errChan <- fmt.Errorf("chunk verification failed at offset %d: %w", chunk.offset, err)
			return
		}
	}
}
