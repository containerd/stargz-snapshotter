...
// IndexConvertFunc is the converter patched to support zstd.
func IndexConvertFunc(layerConvertFunc converter.ConvertFunc, docker2oci bool, platformMC platforms.MatchComparer) converter.ConvertFunc {
	c := &defaultConverter{
...
func (c *defaultConverter) convertManifest(ctx context.Context, cs content.Store, desc ocispec.Descriptor) (*ocispec.Descriptor, error) {
	var (
		manifest converter.DualManifest
		modified bool
	)
...
// TODO: upstream this to github.com/containerd/containerd/image
func getDiffID(ctx context.Context, cs content.Store, desc ocispec.Descriptor) (digest.Digest, error) {
	...
	v, ok := info.Labels[ctdlabels.LabelUncompressed]
	if ok {
	...
	info.Labels[ctdlabels.LabelUncompressed] = digest.String()
	if _, err := cs.Update(ctx, info, "labels"); err != nil {
		logrus.WithError(err).Warnf("failed to set %s label for %s", ctdlabels.LabelUncompressed, desc.Digest)
	}
	return digest, nil
}
