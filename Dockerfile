...
FROM golang:1.25rc3-bullseye AS golang-base

# Build containerd
FROM --platform=$BUILDPLATFORM golang:1.25rc3-bullseye AS containerd-dev
...
FROM --platform=$BUILDPLATFORM golang:1.25rc3-bullseye AS containerd-snapshotter-dev
ARG CONTAINERD_VERSION
ARG TARGETARCH
...
# Build runc
FROM golang:1.25rc3-bullseye AS runc-dev
ARG RUNC_VERSION
...
FROM --platform=$BUILDPLATFORM golang:1.25rc3-bullseye AS snapshotter-dev
ARG TARGETARCH
ARG GOARM
...
FROM golang:1.25rc3-bullseye AS cri-o-dev
ARG CRIO_VERSION
RUN apt-get update -y && apt-get install -y libseccomp-dev libgpgme-dev && \
