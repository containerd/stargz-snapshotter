...
FROM golang:1.27rc3-bookworm AS golang-base

# Build containerd
FROM --platform=$BUILDPLATFORM golang:1.27rc3-bookworm AS containerd-dev
...
FROM --platform=$BUILDPLATFORM golang:1.27rc3-bookworm AS containerd-snapshotter-dev
ARG CONTAINERD_VERSION
ARG TARGETARCH
...
# Build runc
FROM golang:1.27rc3-bookworm AS runc-dev
ARG RUNC_VERSION
...
FROM --platform=$BUILDPLATFORM golang:1.27rc3-bookworm AS snapshotter-dev
ARG TARGETARCH
ARG GOARM
...
# Build CRI-O
FROM golang:1.27rc3-bookworm AS cri-o-dev
ARG CRIO_VERSION
RUN apt-get update -y && apt-get install -y libseccomp-dev libgpgme-dev && \
